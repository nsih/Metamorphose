...
{
    [CustomPropertyDrawer(typeof(ItemAbility), true)]
    public class ItemAbilityDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (EditorGUI.DropdownButton(typeRect, new GUIContent(displayName ?? "Select ItemAbility Type"), FocusType.Keyboard))
            {
                var menu = new GenericMenu();
                if (typeMap == null || typeMap.Count == 0)
                {
                    menu.AddDisabledItem(new GUIContent("No Valid ItemAbility Type found"));
                    menu.ShowAsContext();
                    return;
                }
        ...
        {
            var baseType = typeof(ItemAbility);
